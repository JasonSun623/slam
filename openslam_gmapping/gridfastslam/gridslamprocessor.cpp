...
const double m_distanceThresholdCheck = 20; // Robot jump threshhold
 
using namespace std;

GridSlamProcessor::GridSlamProcessor(): m_infoStream(cout){  
  period_ = 5.0;          //  records the scanning period of the lidar
  m_obsSigmaGain=1;       //  describes the smoothing factor of likelihood
  m_resampleThreshold=0.5;//  determine whether resampling is needed
  m_minimumScore=0.;      //  evaluate whether the scan matching result is ideal Minimum score.
}
...
void GridSlamProcessor::init(unsigned int size, double xmin, double ymin, double xmax, double ymax, double delta, 
  ...
  // Intialize map data: (x,y)min, (x,y)max, delta
  m_xmin=xmin;
  m_ymin=ymin;
  m_xmax=xmax;
  m_ymax=ymax;
  ...
  for (unsigned int i=0; i<size; i++){
    m_particles.push_back(Particle(lmap));        // Set particle map
    m_particles.back().pose=initialPose;          // Set particle pose
    m_particles.back().previousPose=initialPose;  // Set particle previous pose
    m_particles.back().setWeight(0);              // Set particle weight
    m_particles.back().previousIndex=0;           // Set particle previous index. 
    //previousIndex record the particle index that evolved from this particle in the previous generation particle collection.
...
bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){    
  ...
  if (! m_count 
    ...
    // Check if it is not the first read
    if (m_count>0){
